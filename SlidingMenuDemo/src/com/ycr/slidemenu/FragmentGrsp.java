...
import com.ycr.choosevideos.Bimp;
import com.ycr.choosevideos.ChooseVideosActivity;
...
import com.ycr.util.ImageCaesium;
import com.ycr.util.PublicJsonParse;
import com.ycr.util.ReadImgToBinary;

public class FragmentGrsp extends Fragment{
				...
				holder.iv.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
							...
							if(flag){
								...
								item.setSelected(false);
								if(item.getMediaSUrl().equals("add")){
									Intent intent = new Intent(getActivity(),ChooseVideosActivity.class);
									...
									String videoPath=item.getMediaUrl();
									String strend=null; 
							          ...
							          if(strend!=null){
							        	  ...
							        	  Toast toast = Toast.makeText(getActivity().getApplicationContext(),"视频路径错误,不能播放", Toast.LENGTH_SHORT);
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case 0:
				Toast.makeText(getActivity(), "请选择要删除的视频", 400).show();
				break;
			default:
				break;
			}
	...
	private void updateMedia() {
 		...
 		try {
 			...
 			for (List<String> video : Bimp.bmp) {
 				UpPhotoInfo photo=new UpPhotoInfo();
 				Bitmap bm=caes.compressImage(video.get(0));
 				photo.setPhotoUrl(ReadImgToBinary.encodeBase64File(video.get(1)));
 				photo.setCover(ReadImgToBinary.imgToBase64(null,bm,null));
 				images.add(photo);
 			}
 			Gson gson=new Gson();
 			JSONObject json=new JSONObject();
 		...
 		http.send(HttpRequest.HttpMethod.POST,App.service+"updateMedia",params,
 		    new RequestCallBack<String>(){
				public void onSuccess(ResponseInfo<String> responseInfo) {
 		        	...
 		        	Bimp.bmp=new ArrayList<List<String>>();
