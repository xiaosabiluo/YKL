...
public class AlbumHelper {
	...
	HashMap<String, VideoBucket> bucketList = new HashMap<String, VideoBucket>();
...
*/
			do {
				...
				if (bucket == null) {
					...
					bucket.imageList = new ArrayList<VideoItem>();
		...
		Iterator<Entry<String, VideoBucket>> itr = bucketList.entrySet()
				.iterator();
		while (itr.hasNext()) {
			Map.Entry<String, VideoBucket> entry = (Map.Entry<String, VideoBucket>) itr
					.next();
			VideoBucket bucket = entry.getValue();
			Log.d(TAG, entry.getKey() + ", " + bucket.bucketName + ", "
					+ bucket.count + " ---------- ");
			for (int i = 0; i < bucket.imageList.size(); ++i) {
				VideoItem image = bucket.imageList.get(i);
	 ...
	 * @return
	 */
	public List<VideoBucket> getImagesBucketList(boolean refresh) {
